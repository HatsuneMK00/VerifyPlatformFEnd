...
export default {
  ...
  data() {
    return {
      verifyId: '2da68c3ab43f420e95446d07993340b5',
      tableData: [{
        epsilon: '0.01',
        robust_number: '1',
        unrobust_number: '0',
  ...
  created: function() {
    ...
    verification(this.verifyId).then((res) => {
      if (res.status === 200) {
        this.tableData[0].epsilon = res.data.result.eps_0
        this.tableData[0].robust_number = res.data.result.robust_number
        this.tableData[0].unrobust_number = res.data.result.unrobust_number
        this.tableData[0].has_adv_false_number = res.data.result.has_adv_false_number
        this.tableData[0].misclassified_number = res.data.result.misclassified_number
        this.tableData[0].total_runtime = res.data.result.total_runtime
        for (let i = 0; i < res.data.advExamples.length - 1; i++) {
          this.srcs.push({ original: 'http://219.228.60.69:9090/winr/adv_image/' + res.data.advExamples[i], adversarial: 'http://219.228.60.69:9090/winr/adv_image/' + res.data.advExamples[i + 1] })
        }
      } else {
